"""
Bayesian Optimization example for Gaussian Process Regression.
We reuse the same synthetic 1D regression task as the grid/random examples.
"""
...
# 2. objective function (same as before: minimize RMSE)
def objective(params: dict) -> float:
    ...
    print(f"[BO] tried {params} → RMSE={rmse:.4f}")
    return rmse


# 3. hyperparameter grid (same ranges as grid/random search)
param_grid = {
...
best_params, best_score, history = learner.bayesian_search(
    n_init=4,     # initial random evaluations
    n_iter=12,    # BO iterations
